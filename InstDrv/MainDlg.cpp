...
LRESULT CMainDlg::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/) {
	...
	// set icons
	HICON hIcon = AtlLoadIconImage(IDI_MAIN, LR_DEFAULTCOLOR, ::GetSystemMetrics(SM_CXICON), ::GetSystemMetrics(SM_CYICON));
	SetIcon(hIcon, TRUE);
...
LRESULT CMainDlg::OnInstall(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	bool success = InitParams(sysPath, serviceName, displayName);
	if (!success)
		return FALSE;
	SysManager SysMgr(sysPath, serviceName, displayName);
